...
- Uložení sestavy na uživatele + den (`MsFantasyLineup`, `salarySpent`, `pickIds`), uzávěrka podle `lockAt` (viz program v `matches`). Smazání hracího dne s odevzdanými sestavami v PostgreSQL **selže** (FK `gameDayId` → `ON DELETE RESTRICT`), takže lineupy do soutěže se omylem nesmažou kaskádou.
...
- Import soupisek a hracích dnů přes Prisma seed: **`MS_FANTASY_SEED_FANTASY_DATA=true`** doplní / aktualizuje **17 hracích dnů + program** z `src/lib/ms2026FantasyOfficialGameDays.ts` přes **`upsert` podle `slug`** (řádek dne se nemaže, jen se aktualizuje). Následně se defaultně přepíše celý fantasy pool z manifestu v `prisma/seed.ts` (smazání poolu + import JSON repre). Jen kalendář bez změny poolu: **`MS_FANTASY_SEED_FANTASY_DATA_SKIP_POOL=true`** současně s předchozím. Soubor `data/ms2026-fantasy-game-days.json` slouží jen jako rychlý přehled slugů a uzávěrek mimo seed. Jednotlivé týmy lze dál seedovat env `MS_FANTASY_SEED_AUT` apod.; `MS_FANTASY_SEED_SAMPLE` jen doplní dva ukázkové hrací dny, pokud v DB žádné nejsou (ne fantasy pool — ten je jen z JSON repre; při seedu se mažou záznamy `SAMPLE-*`).
- **Úprava poolu během turnaje** (bez rozbití odevzdaných sestav): uprav `data/*-ms2026-fantasy-roster.json`, nové hráče přidej přes `importMsFantasyRosterJson` (nové `code` → nové `id`). Hráče, kteří už nejsou na soupisce MS, **nesmaž** — nastav `active: false` (`deactivateMsFantasyRosterPlayerByCode`). Uzamčené lineupy drží `pickIds` + snímek `pickTiers` / `pickSalaries`. Příklad Rakousko: `npm run fantasy:patch-aut`.
