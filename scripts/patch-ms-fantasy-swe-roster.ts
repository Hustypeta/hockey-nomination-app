/**
 * SWE — sjednocení s IIHF kádrem MS 2026 (25 hráčů).
 *
 *   npm run fantasy:patch-swe
 */
...
const ROSTER_FILE = "sweden-ms2026-fantasy-roster.json";
...
async function main() {
  ...
  await importMsFantasyRosterJson(prisma, "SWE", ROSTER_FILE, "fantasy:patch-swe");

  const legacy = ["SWE26-F92-HEDQVIST"];
  ...
  const active = await prisma.msFantasyRosterPlayer.count({ where: { team: "SWE", active: true } });
  console.log(`Hotovo. Aktivních SWE: ${active} (očekáváno ${rows.length}).`);
