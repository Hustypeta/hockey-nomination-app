/**
 * USA — sjednocení s IIHF kádrem MS 2026 (25 hráčů).
 *
 *   npm run fantasy:patch-usa
 */
...
const ROSTER_FILE = "usa-ms2026-fantasy-roster.json";
...
async function main() {
  ...
  await importMsFantasyRosterJson(prisma, "USA", ROSTER_FILE, "fantasy:patch-usa");

  const legacy = ["USA26-F86-LEE"];
  ...
  const active = await prisma.msFantasyRosterPlayer.count({ where: { team: "USA", active: true } });
  console.log(`Hotovo. Aktivních USA: ${active} (očekáváno ${rows.length}).`);
