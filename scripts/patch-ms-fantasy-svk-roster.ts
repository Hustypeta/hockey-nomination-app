/**
 * SVK — sjednocení s IIHF kádrem MS 2026 (25 hráčů).
 *
 *   npm run fantasy:patch-svk
 */
...
const ROSTER_FILE = "slovakia-ms2026-fantasy-roster.json";
...
async function main() {
  ...
  await importMsFantasyRosterJson(prisma, "SVK", ROSTER_FILE, "fantasy:patch-svk");

  const legacy = ["SVK26-D00-PETROVI", "SVK26-F00-LACKA", "SVK26-F00-PETRAS"];
  ...
  const active = await prisma.msFantasyRosterPlayer.count({ where: { team: "SVK", active: true } });
  console.log(`Hotovo. Aktivních SVK: ${active} (očekáváno ${rows.length}).`);
