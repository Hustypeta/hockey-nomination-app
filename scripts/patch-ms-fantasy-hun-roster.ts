/**
 * HUN — sjednocení s IIHF kádrem MS 2026 (25 hráčů).
 *
 *   npm run fantasy:patch-hun
 */
...
const ROSTER_FILE = "hungary-ms2026-fantasy-roster.json";
...
async function main() {
  ...
  await importMsFantasyRosterJson(prisma, "HUN", ROSTER_FILE, "fantasy:patch-hun");
  ...
  const active = await prisma.msFantasyRosterPlayer.count({ where: { team: "HUN", active: true } });
  console.log(`Hotovo. Aktivních HUN: ${active} (očekáváno ${rows.length}).`);
