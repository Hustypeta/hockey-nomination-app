/**
 * LAT — sjednocení s IIHF kádrem MS 2026 (25 hráčů).
 *
 *   npm run fantasy:patch-lat
 */
...
const ROSTER_FILE = "latvia-ms2026-fantasy-roster.json";
...
async function main() {
  ...
  await importMsFantasyRosterJson(prisma, "LAT", ROSTER_FILE, "fantasy:patch-lat");
  ...
  const active = await prisma.msFantasyRosterPlayer.count({ where: { team: "LAT", active: true } });
  console.log(`Hotovo. Aktivních LAT: ${active} (očekáváno ${rows.length}).`);
