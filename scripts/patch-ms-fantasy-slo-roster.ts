/**
 * SLO — sjednocení s IIHF kádrem MS 2026 (25 hráčů).
 *
 *   npm run fantasy:patch-slo
 */
...
const ROSTER_FILE = "slovenia-ms2026-fantasy-roster.json";
...
async function main() {
  ...
  await importMsFantasyRosterJson(prisma, "SLO", ROSTER_FILE, "fantasy:patch-slo");

  const stale = await prisma.msFantasyRosterPlayer.findMany({
    where: { team: "SLO", active: true },
  ...
  const active = await prisma.msFantasyRosterPlayer.count({ where: { team: "SLO", active: true } });
  console.log(`Hotovo. Aktivních SLO: ${active} (očekáváno ${rows.length}).`);
