...
/**
 * Odeslání Pick’em tipů k účtu (upsert — jeden záznam na uživatele).
 */
export async function POST(request: NextRequest) {
  try {
    const session = await getServerSession(authOptions);
    if (!session?.user?.id) {
      return NextResponse.json({ error: "Pro odeslání Pick’emu se musíš přihlásit." }, { status: 401 });
    }
    ...
    // Nevalidujeme do detailu (payload už je klientsky omezený + verze); ukládáme celý objekt.
    ...
    console.error("POST /api/pickem/submit:", e);
    ...
    return NextResponse.json({ error: "Odeslání Pick’emu se nepovedlo." }, { status: 500 });
  }
}
