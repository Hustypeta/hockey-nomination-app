...
export function tryAutoAssignPlayer(lineup: LineupStructure, player: Player): LineupStructure | null {
  ...
  let [a, b] = next.extraForwards;
  if (!a) {
    next.extraForwards = [player.id, b];
    return next;
  }
  if (!b) {
    next.extraForwards = [a, player.id];
    return next;
  }
  return null;
}
...
export function buildRandomLineup(players: Player[]): LineupStructure | null {
  ...
  const extraForwards: LineupStructure["extraForwards"] = [fs[fi].id, fs[fi + 1].id];
