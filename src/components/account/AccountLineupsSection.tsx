...
export function AccountLineupsSection() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([
      fetch("/api/account/match-share-links", { credentials: "include", cache: "no-store" }).then((r) => {
      ...
      .then(([linksData, playerList]) => {
        if (cancelled) return;
        setLineups((linksData as { links?: MatchLineupRow[] }).links ?? []);
  ...
  return (
    <section className="fifa-account-section">
      <div className="fifa-account-section__head">
        ...
        <Link href="/zapasy/sestava" className={`${FIFA_BTN_PRIMARY} fifa-account-section__action`}>
      ...
      {loadError ? (
        ...
        <p className="fifa-account-section__empty">
          Zatím nemáš uložené sestavy.{" "}
          <Link href="/zapasy/sestava" className="text-[var(--fifa-accent-text)] hover:underline">
        ...
        <div className="fifa-account-lineup-tiles">
          {lineups.map((row) => {
            ...
            return (
              <Link
                key={row.code}
                href={`/zapasy/sestava?kod=${encodeURIComponent(row.code)}`}
