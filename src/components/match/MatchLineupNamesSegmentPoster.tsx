...
export const MatchLineupNamesSegmentPoster = forwardRef<HTMLDivElement, MatchLineupNamesSegmentPosterProps>(
  function MatchLineupNamesSegmentPoster(
    ...
    const gridCols =
      group === "goalies" ? "grid-cols-3" : group === "defense" ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-3";

    return (
      <div
        ref={ref}
        className="match-lineup-names-segment-poster relative shrink-0 overflow-hidden rounded-none border-0 bg-[#060b14] shadow-[0_24px_70px_rgba(0,0,0,0.45)] antialiased [text-rendering:optimizeLegibility]"
      ...
      >
        ...
        <div className="relative z-[1] px-9 pb-6 pt-10 sm:px-11 sm:pb-8 sm:pt-11">
          <header className="max-w-[62%] pr-2">
            ...
            <p
              className={`font-display text-[11px] font-semibold uppercase tracking-[0.22em] text-white/52 ${titleLine ? "mt-2" : "mt-3"}`}
            >
              Český nároďák · soupiska jmen · {MATCH_LINEUP_POSTER_GROUP_TITLE[group]}
            </p>
          </header>
          ...
          <div className="mt-9 sm:mt-10">
            <section>
              ...
              <div className={`mx-auto grid max-w-3xl gap-2.5 ${gridCols} sm:gap-3`}>
                {names.map((name, i) => (
